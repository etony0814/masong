...
app.post('/api/photos', requireAuth, photoUpload.single('photo'), (req, res) => {
  ...
  const id = db.insertAndGetId(
    'INSERT INTO photos (memory_id, filename, caption, order_index, drive_url) VALUES (?, ?, ?, ?, ?)',
    [memory_id || null, req.file.filename, caption || '', order_index || 0, '']
  );
  res.json({ id, filename: req.file.filename, drive_url: '' });
});

// 影片上傳（本地儲存，drive_url 設為空）
app.post('/api/videos', requireAuth, videoUpload.single('video'), (req, res) => {
  const { memory_id, caption, duration } = req.body;
  if (!req.file) return res.status(400).json({ error: '沒有檔案' });
  const id = db.insertAndGetId(
    'INSERT INTO videos (memory_id, filename, caption, duration, drive_url) VALUES (?, ?, ?, ?, ?)',
...
// 雲端照片上傳（Google Drive）
app.post('/api/cloud/photos', requireAuth, photoUpload.single('photo'), (req, res) => {
  const { memory_id, caption, order_index } = req.body;
  if (!req.file) return res.status(400).json({ error: '沒有檔案' });
  driveService.uploadToDrive(req.file.filename, req.file.buffer, req.file.mimetype)
    .then(url => {
      const id = db.insertAndGetId(
        'INSERT INTO photos (memory_id, filename, caption, order_index, drive_url) VALUES (?, ?, ?, ?, ?)',
        [memory_id || null, req.file.filename, caption || '', order_index || 0, url]
      );
      res.json({ id, filename: req.file.filename, drive_url: url });
    })
    .catch(err => {
      console.error('Google Drive 上傳失敗:', err);
      res.status(500).json({ error: 'Google Drive 上傳失敗: ' + err.message });
    });
});

// 雲端影片上傳（Google Drive）
app.post('/api/cloud/videos', requireAuth, videoUpload.single('video'), (req, res) => {
  const { memory_id, caption, duration } = req.body;
  if (!req.file) return res.status(400).json({ error: '沒有檔案' });
  driveService.uploadToDrive(req.file.filename, req.file.buffer, req.file.mimetype)
    .then(url => {
      const id = db.insertAndGetId(
        'INSERT INTO videos (memory_id, filename, caption, duration, drive_url) VALUES (?, ?, ?, ?, ?)',
        [memory_id || null, req.file.filename, caption || '', duration || '', url]
      );
      res.json({ id, filename: req.file.filename, drive_url: url });
    })
    .catch(err => {
...
app.delete('/api/photos/:id', requireAuth, (req, res) => {
  const photo = db.get('SELECT filename, drive_url FROM photos WHERE id=?', [req.params.id]);
  if (photo) {
    if (photo.drive_url) {
      // 從 Drive 刪除
      const match = photo.drive_url.match(/id=([a-zA-Z0-9_-]+)/);
      if (match) driveService.deleteFromDrive(match[1]).catch(console.error);
    } else {
      try { fs.unlinkSync(path.resolve(__dirname, 'uploads', 'photos', photo.filename)); } catch (e) {}
    }
  }
...
app.delete('/api/videos/:id', requireAuth, (req, res) => {
  const video = db.get('SELECT filename, drive_url FROM videos WHERE id=?', [req.params.id]);
  if (video) {
    if (video.drive_url) {
      const match = video.drive_url.match(/id=([a-zA-Z0-9_-]+)/);
      if (match) driveService.deleteFromDrive(match[1]).catch(console.error);
    } else {
      try { fs.unlinkSync(path.resolve(__dirname, 'uploads', 'videos', video.filename)); } catch (e) {}
    }
  }
...
app.post('/api/import/backup', requireAuth, backupUpload.single('backup'), async (req, res) => {
  try {
    ...
    zip.getEntries()
      ...
      .forEach(entry => {
        const uploadPath = entry.entryName;
